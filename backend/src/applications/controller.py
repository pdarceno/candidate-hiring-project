...
def apply_to_job(application_data: model.ApplicationCreate, db: DBSession, _: CurrentUser):
    return service.apply_to_job(db, application_data)
...
def update_application_status(application_id: UUID, status_update: model.ApplicationUpdate, db: DBSession, _: CurrentUser):
